<nav class="flex items-start border-black main-nav md:border-b-2">
    ...
    <ul id="nav" class="flex overflow-hidden absolute top-0 left-0 z-10 flex-col flex-wrap gap-y-6 pt-0 pr-4 pb-0 pl-4 w-full max-h-0 text-3xl bg-white shadow-2xl opacity-0 transition-opacity md:bg-transparent md:static md:max-h-full md:opacity-100 md:gap-x-14 md:flex-row md:gap-y-4 md:shadow-none md:py-6 md:w-auto md:pl-0">
...
<script>
    ...
    document.querySelector('#toggle-nav').addEventListener('click', () => {
        ...
        document.querySelector('#nav').classList.toggle('pt-0')
        document.querySelector('#nav').classList.toggle('pb-0')
        document.querySelector('#nav').classList.toggle('pt-20')
        document.querySelector('#nav').classList.toggle('pb-8')
    })
